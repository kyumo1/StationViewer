...
class GetStationsUseCase @Inject constructor(private val stationsRepository: IStationsRepository) :
    IGetStationsUseCase {
    override fun get(lineCode: Int): Observable<List<StationOutputData>> {
        return stationsRepository.get(lineCode)
            .map { data -> data.map { s -> StationOutputData(s.code, s.name, s.longitude, s.latitude) } }
    }
}